...
my_name = "--SUJAL--"
CHANNEL_ID = "-1002710444355"##change it with your channel 🆔 
...
async def root_route_handler(request):
    return web.json_response("https://txt-to-video-21kx.onrender.com") ## change it with your host url
...
async def txt_handler(bot: Client, m: Message):
    ...
    await input.delete(True)
    file_name, ext = os.path.splitext(os.path.basename(x))
    credit =credit = "ＳＵＪＡＬ"  # बिना emoji या ™
    try:    
        with open(x, "r") as f:
            content = f.read()
        content = content.split("\n")
